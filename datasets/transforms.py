...
class NormalizeAndPad(object):
    ...
    def __call__(self, img, target):
        ...
        if self.padding:
            out_img_ori = torch.zeros((3, self.size, self.size)).float()
            out_img_ori[:, top:top + h, left:left + w] = img_ori
            out_mask = torch.zeros((self.size, self.size), dtype=torch.bool)
        else:
            out_img_ori = img_ori
        ...
        out_h, out_w = out_img_ori.shape[-2:]
        box = box_xyxy_to_cxcywh(box)
        box = box / torch.tensor([out_w, out_h, out_w, out_h], dtype=torch.float32)
        return out_img,out_img_ori, target
