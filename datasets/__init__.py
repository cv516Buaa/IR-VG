...
train_transforms = [
    dict(
        type='RandomSelect',
        transforms1=dict(type='RandomResize', sizes=[640], record_resize_info=True),
        transforms2=dict(
            type='Compose',
            transforms=[
                dict(type='RandomResize', sizes=[400, 500, 600], resize_long_side=False, record_resize_info=True),
                dict(type='RandomSizeCrop', min_size=384, max_size=600, check_method=dict(func='iou', iou_thres=0.5)),
                dict(type='RandomResize', sizes=[448, 480, 512, 544, 576, 608, 640], record_resize_info=True)
            ],
        ),
        p=0.5
    ),
