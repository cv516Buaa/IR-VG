...
class VGDataset(Dataset):
    ...
    def getitem__PIL(self, idx):
        ...
        features, keys, keys_mask = convert_examples_to_features(
            examples=examples, seq_length=self.query_len, tokenizer=self.tokenizer)
        word_id = features[0].input_ids
        word_mask = features[0].input_mask
        target['keys'] = torch.tensor(keys, dtype=torch.long)
        target['keys_mask'] = torch.tensor(keys_mask, dtype=torch.long)
        target['word_id'] = torch.tensor(word_id, dtype=torch.long)
        target['word_mask'] = torch.tensor(word_mask, dtype=torch.bool)
        target['idx'] = torch.tensor(idx, dtype=torch.long)
        if 'mask' in target:
