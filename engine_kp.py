...
class data_prefetcher():
    ...
    def preload(self):
        ...
        with torch.cuda.stream(self.stream):
            ...
            self.next_target.tensor_dict = {k: tensor_dict[k].to(self.device, non_blocking=True) for k in tensor_dict if k != 'phrase'}
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module, postprocessor,
    ...
    while img is not None:
        ...
        outputs = model(img, mask, word_id, word_mask, 0, target_dict, postprocessor, target_dict['keys'], target_dict['keys_mask'])
...
def train_one_epoch_w_accum(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    while img is not None:
        ...
        for i in range(2):
            ...
            outputs = model(b_img, b_mask, b_word_id, b_word_mask, 0)
...
def evaluate(model, criterion, postprocessor, data_loader, device, save_path=''):
    ...
    for iteration, (img, mask, target) in enumerate(tqdm(prefetcher)):
        ...
        outputs = model(img, mask, word_id, word_mask, target_dict['idx'], target_dict, postprocessor, target_dict['keys'], target_dict['keys_mask'])
        kp = outputs['kp']
        criterion = False
        if criterion:
            loss_dict = criterion(outputs, target_dict)
        ...
        pred = pred_boxes[0]['boxes'].cpu().numpy().astype(int)
        gt = gt_bbox.cpu().numpy().astype(int)
        ious = box_ops.box_pair_iou(gt_bbox, pred_boxes[0]['boxes'])[0]
